...
export default async function DashboardOverview() {
  ...
  let recentTopStories = await prisma.article.findMany({
    where: { status: 'COMPLETED' },
  ...
  // Fallback to un-processed articles if no completed ones exist
  if (recentTopStories.length === 0) {
    recentTopStories = await prisma.article.findMany({
      ...
      <div>
        ...
        {recentTopStories.length === 0 ? (
          <div className="glass-card flex flex-col items-center justify-center py-16 px-4 text-center">
            ...
            <h4 className="text-lg font-medium text-white mb-2">No stories ingested yet</h4>
              ...
              <Card key={story.id} className="glass-card group">
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-start mb-2">
                    <Badge variant="secondary" className="bg-violet-500/10 text-violet-400 hover:bg-violet-500/20 border border-violet-500/20 shadow-[0_0_10px_rgba(139,92,246,0.1)]">
                      {story.category || (story.status !== 'COMPLETED' ? "Pending AI Processing" : "Uncategorized")}
                    </Badge>
                ...
                <CardContent>
                  <p className="text-sm text-gray-300/80 line-clamp-3 mb-4">
                    {story.summary || story.contentSnippet || "Pending AI processing..."}
                  </p>
